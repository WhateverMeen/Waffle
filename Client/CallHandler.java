...
public class CallHandler extends Thread{
    ...
    public void connect_peer(String username, String peerIp){
        ...
        try {
            ...
            MediaStream audioStream = mediaservice.createMediaStream(captureDevice);
    ...
    public void run(){
        try {
            running = true;
            while (running){
                String in = EncryptionManager.decrypt_message(notify_in.readLine(), client_private_key);
                if (in[0].equals("INCOMING")){
                    client.notify_call_incoming(in[1]);
